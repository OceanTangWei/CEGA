...
def read_tex_graph(file_name):
    ...
    elif file_name == "arxiv":
        path = r"./dataset/Arxiv1-Arxiv2/Arxiv_G1.edgelist"
    elif file_name == "blog":
        path = r"./dataset/blog/blogedges.csv"
    else:
        raise ValueError
    if file_name!="arxiv":
        g = nx.Graph()
...
def compute_edge_overlap_rate(G1,G2):
...
def sparse_sinkhorn_sims_trainable(e_left,e_right, top_k=500,iteration=15,mode = "test"):
    
    gpus = tf.config.experimental.list_physical_devices(device_type="GPU")
    faiss.normalize_L2(e_left); faiss.normalize_L2(e_right)

    res = faiss.StandardGpuResources()
    dim, measure = e_left.shape[1], faiss.METRIC_INNER_PRODUCT
    if mode == "test":
        param = 'Flat'
    ...
    index.train(e_right)
    index.add(e_right)
    sims, index = index.search(e_left, top_k)
    
    
    row_sims = K.exp(sims.flatten()/0.02)
    index = K.flatten(index.astype("int32"))
    size = e_left.shape[0]
    row_index = K.transpose(([K.arange(size*top_k)//top_k,index,K.arange(size*top_k)])).numpy()
    row = row_index[:,0]
    col = row_index[:,1]
